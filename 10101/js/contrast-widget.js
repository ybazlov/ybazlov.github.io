...
function switchSlides(newstyle){
	// change the SRC of the images
	var slidelist = this.document.getElementsByClassName('slide');
	var goods = "images/b"; var bads = "images/db";
	if(newstyle.includes('darkStyle')) { bads = goods; goods = "images/db"; }
	for(var k=0; k<slidelist.length; k++) {
		var s = slidelist[k].src;
		if(s.includes(bads)) {
			// console.log("s=",s," bads=", bads, " goods=", goods);
			slidelist[k].src = s.replace(bads,goods); 
			// console.log("now s=",s);
		}
	}
}
